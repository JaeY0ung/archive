...
const updateUserInfo = async () => {
  if (loginUserInfo.value.nickname !== originalUserInfo.value.nickname && (!isNicknameChecked.value || !isNicknameAvailable.value)) {
  ...
  try {
    ...
    if (loginUserInfo.value.profileImage && loginUserInfo.value.profileImage !== originalUserInfo.value.profileImage) {
...
const isModified = computed(() => {
  return loginUserInfo.value.nickname !== originalUserInfo.value.nickname ||
...
<template>
  <div class="container">
    ...
    <div class="form-control w-full mb-4">
      ...
      <input v-model="loginUserInfo.email" type="text" class="input input-bordered w-full" disabled />
    </div>
    ...
    <div class="form-control w-full mb-4">
      ...
      <div class="flex">
        ...
        />
        <button @click="checkNicknameDuplicate" class="btn btn-outline" :disabled="!isModified || loginUserInfo.nickname === originalUserInfo.nickname">중복확인</button>
      </div>
    </div>
    ...
    <div class="form-control w-full mb-4">
      ...
      <div class="flex items-center">
        <input 
          type="text" 
          :value="loginUserInfo.profileImage ? loginUserInfo.profileImage.name : '선택된 파일 없음'" 
          class="input input-bordered flex-grow mr-2" 
          readonly 
        />
