...
const onStartRecordingEmit = () => {
  stompClient.send(`/app/play/start/${route.params.roomId}`, {}, JSON.stringify({
    type: "start",
    sender: loginUser.nickname,
    content: "start",
    sheetId: route.params.sheetId,
    resultId: multiResultId
  }));
  startRecording();
}
...
// refactoring은 나중에.
const sendExit = () => {
  stompClient.send(`/app/wait/start/${route.params.roomId}`, {}, JSON.stringify({
    type: "exit",
    sender: loginUser.nickname,
    content: 'true',
    sheetId: 0
...
const sendEndDuringPlay = () => {
  stompClient.send(`/app/play/end/quit/${route.params.roomId}`, {}, JSON.stringify({
    sender: loginUser.nickname,
        ...
        stompClient.subscribe(`/play/start/socket/${route.params.roomId}`, async (socket) => {
          ...
          if (loginUser.nickname != message.sender) {
            ...
            try {
              ...
              stompClient.send(`/app/play/id/${route.params.roomId}`, {}, JSON.stringify({
                sender: loginUser.nickname,
                resultId: multi_result_id
              }))
              isResultSender = true;
