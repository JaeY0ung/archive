...
import { searchSheetsByFilter, searchSheetByStatusForAdmin } from "@/api/sheet";
...
const changeSheetStatusBySheetId = (sheetId, status) => {
    local.put(`/sheets/${ sheetId }/status/${ status }`)
        .then((res) => {
            searchSheets();
        })
}
...
<template>
    <div class="m-auto">
        <div class="flex justify-center gap-3">
                        ...
                        <div class="flex">
                            <SmallSheetCard :sheet v-if="sheet.id != updatingSheetId">
                                <div class="h-full flex flex-col items-center gap-1">
                                    <div class="bg-blue-500 rounded-xl p-2 cursor-pointer" @click="changeSheetStatusBySheetId(sheet.id, 1)">승인</div>
                                    <div class="bg-red-500 rounded-xl p-2 cursor-pointer" @click="changeSheetStatusBySheetId(sheet.id, 2)">거절</div>
                                </div>
                            </SmallSheetCard>
            ...
            <div class=" bg-blue-700 rounded-xl p-1">
                ...
                <div>
                    <template v-for="sheet in sheets[1]" :key="sheet.id">
                        <div class="flex">
                            <SmallSheetCard :sheet v-if="sheet.id != updatingSheetId">
                                <div class="h-full flex flex-col items-center gap-1">
                                    <div class="bg-green-500 rounded-xl p-2 cursor-pointer" @click="changeSheetStatusBySheetId(sheet.id, 0)">대기</div>
                                    <div class="bg-red-500 rounded-xl p-2 cursor-pointer" @click="changeSheetStatusBySheetId(sheet.id, 2)">거절</div>
            ...
            <div class=" bg-red-700 rounded-xl p-1">
