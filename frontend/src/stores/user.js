...
export const useUserStore = defineStore(
    ...
    () => {
        ...
        messaging.onMessage((payload) => {
            ...
            const receiverId = payload.data.receiverId ? parseInt(payload.data.receiverId, 10) : null;
            const senderId = payload.data.senderId ? parseInt(payload.data.senderId, 10) : null;

            // 현재 로그인된 사용자의 ID를 가져옴
            const currentUserId = userInfo.value.id;

            // Sender와 Receiver가 동일한 경우 알림을 무시함
